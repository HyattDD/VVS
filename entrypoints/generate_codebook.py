...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default="lumina_mgpt",
                        help="Model type; choices: ['lumina_mgpt']")
    parser.add_argument('--save_path', type=str,
                        default="ckpts/lumina_mgpt/vq_distances", help="Path to save the codebook")
...
def run_generate_codebook(args):
    ...
    latents = vq_model.quantize.embedding.weight  # (16348, 256)

    distances = torch.cdist(latents, latents, p=2)  # (16348, 16348)
    ...
    np.save(os.path.join(args.save_path,
            f"top_{k}_indices.npy"), topk_indices_uint16.cpu().detach().numpy())
