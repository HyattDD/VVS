...
def parse_args():
    ...
    parser.add_argument(
        "--ref_dir",
        required=True,
        default="/home/COCO/val2017",
        help="location of the reference images for evaluation",
    )
    parser.add_argument(
        "--caption_path", default="data/prompts/captions_val2017_longest.json"
    )

    parser.add_argument("--how_many", default=500, type=int)
    parser.add_argument(
        "--clip_model4eval",
        default="ViT-B/32",
        type=str,
        help="[WO, ViT-B/32, ViT-G/14]",
    )
...
class EvalDataset(Dataset):
    def __init__(
        ...
        if isinstance(self.resize_size, int):
            self.trsf_list += [
                transforms.Resize(
                    self.resize_size, interpolation=resizer_collection[resizer]
                )
            ]
        if self.normalize:
    ...
    def load_dataset(self):
        self.imagelist = self.natural_sort(
            glob.glob(os.path.join(self.data_dir, "*.%s" % "png"))
        )
...
def compute_clip_score(
    ...
    for imgs, txts in tqdm(dataset):
        ...
        new_text_tokens = torch.cat(
            [start_tokens, prepend_text_tokens, tokens[:, 1:]], dim=1
        )[:, :77]
...
def run_eval_fid_clip(opt):
    ...
    clip_score = compute_clip_score(
        dset2_dataloader, clip_model=opt.clip_model4eval, how_many=opt.how_many
    )
