...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        query_states = apply_rotary_emb(
            query_states.transpose(1, 2), freqs_cis.to(query_states.device)
        ).transpose(1, 2)
        key_states = apply_rotary_emb(
            key_states.transpose(1, 2), freqs_cis.to(key_states.device)
        ).transpose(1, 2)
...
class Model(nn.Module):
    def __init__(
        ...
        super().__init__()
        # import numpy as np #DOG
        # self.top1000_book = np.load('ckpts/llamagen/target_top1000_top1_list.npy') #DOG
        ...
        if load_emb:
            ...
            try:
                ...
                try:
                    ...
                    with safe_open(
                        os.path.join(path, head_path), framework="pt", device="cpu"
                    ) as f:
                    ...
                    weights = torch.load(
                        os.path.join(path, head_path), weights_only=True
                    )
        ...
        self.softmax = nn.Softmax(dim=-1) #DOG
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            # we can also provide embedding out of this function, just like SpecVLA
    ...
    @torch.no_grad()
    def topK_genrate_v2(self, hidden_states, input_ids, head, logits_processor, cfg_scale):
        ...
        # shift 1 step to right, because eagle adopts:
        # [token1, token2, token3] -> [token2, token3, <eos>]
        # [feature0, feature1, feature2] -> [feature1, feature2, <eos>]
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            ...
            position_ids = torch.arange(
                kv_len, input_ids.shape[1], device=hidden_states.device
            ).unsqueeze(0)
        ...
        ss_hidden.append(last_hidden.unsqueeze(1).repeat(1, top_k, 1))  #DOG
        ...
        for i in range(depth):
            ...
            # with Timer("1index"):
            input_ids = topk_index.view(-1)[topk_cs_index][None].repeat(
                input_hidden.shape[0], 1
            )

            ss_token.append(topk_index)  # for global rank and fetch
            scores_list.append(cu_scores) # for global rank and fetch
            tree_mask = torch.cat(
                (
                    tree_mask[:, :, out_ids.to(device=tree_mask.device)],
                    self.tree_mask_init,
                ),
                dim=3,
            )
            
            # input_hidden shape like: [2, 10, 1280], hidden state of global topk tokens
            ss_hidden.append(out_hidden.repeat_interleave(10, dim=1))  #DOG

        # with Timer("post"):

        scores_list = torch.cat(scores_list, dim=0).view(-1)
        ss_token_list = torch.cat(ss_token, dim=0).view(-1)
        ss_hidden_list = torch.cat(ss_hidden, dim=1) #DOG
        top_scores = torch.topk(scores_list, total_tokens, dim=-1)
        top_scores_index = top_scores.indices
        top_scores_index = torch.sort(top_scores_index).values

        draft_tokens = ss_token_list[top_scores_index]
        draft_hiddens = ss_hidden_list[:, top_scores_index, :]  #DOG
        # final draft tokens include the root token generated by the target model
        draft_tokens = torch.cat((sample_token[:1], draft_tokens), dim=0)
        draft_hiddens = torch.cat((hidden_states[:,-1].unsqueeze(1), draft_hiddens), dim=1)  #DOG
        ...
        draft_hiddens = draft_hiddens[None]  #DOG
        ...
        # draft_tokens shape: [1, 59], draft_hiddens shape: [1, 2, 59, 1280]
        return draft_tokens, draft_hiddens, retrieve_indices, tree_mask, tree_position_ids
    ...
    @torch.no_grad()
    def topK_genrate_v1(
        self, hidden_states, input_ids, head, logits_processor, cfg_scale
    ):
        # test_=input_ids
        ...
        for i in range(len(self.tree_buffer["tree_indices"])):
            if logits_processor is not None:
                topk_index, topk_prob, op = self.sample(
                    last_headout, logits_processor, k=self.top_k
                )
            else:
