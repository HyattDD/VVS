...
from professional_profiling_utils import ProfessionalTokenProfiler
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    from_pretrained: classmethod # to let pylance static checking know, can be removed
    ...
    def __init__(self, config):
        ...
        self.t5_model = T5Embedder(
            device = 'cuda',
            local_cache=True,
            cache_dir='/home/htdong/public_net/models/t5',
    ...
    def generate(
        ...
        profiling_save_path: Optional[str] = None,
        **model_kwargs,
    ):
        # 初始化专业的性能分析器
        profiler = None
        if enable_profiling:
            ...
            profiler = ProfessionalTokenProfiler(str(device), enable_chrome_trace=False)
            # 获取prompt内容用于分析
            prompt_text = prompt[0] if isinstance(prompt, list) and len(prompt) > 0 else str(prompt)
            profiler.start_profiling(prompt_text, model=self.model)
            print(f"开始专业性能分析，设备: {device}")
        ...
        for i in range(1, max_length):
            ...
            # professional profiling: per token analysis
            if profiler is not None:
                with profiler.profile_token(i):
        ...
        # end profiling and get summary
        if profiler is not None:
            profiling_summary = profiler.end_profiling()
            
            # save profiling results
            if profiling_save_path:
                profiler.save_results(profiling_save_path)
                print(f"Profiling results are saved to: {profiling_save_path}")
        if profiling_summary:
            return seq, 1.0, time.time() - st, profiling_summary
        else:
            return seq, 1.0, time.time() - begin_time
